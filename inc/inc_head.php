...
$site_url = trim((string)($nova_settings['site_url'] ?? ''));
$project_web_path = '';

if ($site_url !== '') {
  $og_site_url = rtrim($site_url, '/');
} else {
  ...
  $project_web_path = $script_dir;
  $og_site_url = $scheme . '://' . $host . $script_dir;
}
...
if ($project_web_path !== '' && strpos($request_path, $project_web_path) === 0) {
  $request_path = substr($request_path, strlen($project_web_path));
  if ($request_path === '') {
    $request_path = '/';
  }
}

$current_request_uri = $request_query ? $request_path . '?' . $request_query : $request_path;

$og_url = $og_site_url . $current_request_uri;
$og_image = isset($page_image) ? $og_site_url . "/" . ltrim($page_image, '/') : $og_site_url . "/img/logo-star51.png";
?>
<!doctype html>
