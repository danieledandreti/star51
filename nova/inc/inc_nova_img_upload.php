<?php
/**
 * Nova Image Upload - Linear upload processing for gallery images (1, 2)
 * Validates and uploads images without resize (resize happens in separate file)
 */

// Initialize upload result variables
    ...
    if ($image_info === false) {
        ...
        if (!$has_image1_error) {
            ...
            if (move_uploaded_file($file['tmp_name'], $destination)) {
                ...
                error_log("Nova: Gallery image_1 uploaded - File: $filename");
            } else {
...
if (isset($_FILES['image_2']) && $_FILES['image_2']['error'] === UPLOAD_ERR_OK) {
    ...
    if ($image_info === false) {
        ...
        if (!$has_image2_error) {
            ...
            if (move_uploaded_file($file['tmp_name'], $destination)) {
                chmod($destination, NOVA_FILE_CHMOD);
                $uploaded_image_2 = $filename;
                error_log("Nova: Gallery image_2 uploaded - File: $filename");
            } else {
