...
function nova_send_password_reset_email($to, $first_name, $last_name, $token)
{
  ...
  // Build reset URL (prefer configured site_url, fallback to runtime autodetect)
  ...
  if ($site_url !== '') {
    ...
    if (substr($script_dir, -5) === '/nova') {
      $project_dir = substr($script_dir, 0, -5);
    } elseif (($nova_pos = strpos($script_dir, '/nova/')) !== false) {
      $project_dir = substr($script_dir, 0, $nova_pos);
    } else {
      $project_dir = $script_dir;
    }

    $site_url = $protocol . '://' . $host . $project_dir;
  }

  $reset_url = $site_url . '/nova/password-new.php?token=' . urlencode($token);
